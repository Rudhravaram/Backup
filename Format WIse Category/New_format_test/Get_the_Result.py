...
def pdf_to_img(pdfName, folderName):
    if os.path.exists(os.path.join(os.getcwd(), "pdf", str(folderName), str(pdfName))):
        path_s= os.path.join(os.getcwd(), "pdf", str(folderName), str(pdfName))
        print(46)
    pdf = WandImage(filename=os.path.join(os.getcwd(), "pdf", str(folderName), str(pdfName)), resolution=300)
    pdfimage = pdf.convert("pdf")
    i = 1
    pdfNamesss=pdfName.replace('.pdf','').replace('PDF','').replace('.','')
    for img in pdfimage.sequence:
        filename = os.path.join(os.getcwd(), "pdf", str(folderName),pdfNamesss + str(i))
        a = '"' + path_s + '"'
        a1='"' + filename + '"'
...
def Transcript_of_Proposal(fw,file_name,path_s,page_no):
    ...
    # temp_Vehicle_Details = []
    # final_vechile=[]
    # appende_list=[]
    # for i in range(len(lines)):
    #     if str(lines[i]).__contains__("Vehicle Details"):
    #         Vehicle_Details = True
    #     if "Engine Num" in lines[i]:
    #         break
    #     if not Vehicle_Details:
    #         continue
    #     data_list = lines[i].replace('Vehicle Details','').replace(':', '').replace('B.', '').split('          ')
    #     if not len(data_list) > 0:
    #         continue
    #     print(data_list)
    #     data_list = clean_list(data_list)
    #     print(data_list)
    #     temp_Vehicle_Details.append(data_list)
    # for i in range(len(temp_Vehicle_Details)):
    #     if temp_Vehicle_Details[i].__contains__('Sub Type') and (temp_Vehicle_Details[i+1].__contains__('Regn') or temp_Vehicle_Details[i+1].__contains__('Kilowatt')):
    #         for j in range(len(temp_Vehicle_Details[i+2])):
    #             if len(temp_Vehicle_Details[i+3])==2:
    #                 for jj in range(len(temp_Vehicle_Details[i+3])):
    #                     appende_list.append(temp_Vehicle_Details[i + 3][jj])
    #             final_vechile.append(temp_Vehicle_Details[i+2][j])
    # if final_vechile:
    #     if len(final_vechile)==7:
    #         var1=final_vechile[0].split(' ')
    #         dict1['registration_no']=var1[0]
    #         dict1['date_of_registration'] = var1[1]
            # if appende_list:
            #     dict1['registration_no']=
    # for i in range(len(final_vechile)):
...
def CERTIFICATE_CUM_POLICY_FOR_COMMERCIAL(fw,file_name,path_s,page_no):
    ...
    print('================================================Vehicle Model=============================================================')
    if final_res:
        dict1['model'] = final_res[4].replace('- ','')
    else:
        dict1['model'] = ''
    print(dict1['model'])
    print('================================================Cubic Capacity/ Kilowatt=============================================================')
    ...
    for i in range(len(lines)):
        ...
        for i in range(len(data_list)):
            if data_list[i].__contains__('Policy Issued on') and not data_list[-1].__contains__('Policy Issued on') :
                ...
                Address_Insured +=data_list[i]
    if Address_Insured:
        dict1['address'] = Address_Insured.replace(rep_1,'').replace(rep_2,'').replace('Address','')
    else:
        dict1['address'] = ''
    print(dict1['address'])
    print('************************************Pincode*********************************************')
