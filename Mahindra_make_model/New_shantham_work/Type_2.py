...
def Get_final_data():
    file = 'Sample_updated_type2.xlsx'
    ...
    if ws_new_myra:
        ...
        for i in range(sheet_new_myra.nrows):
            ...
            for j in range(sheet_set1_sheet.nrows):
                ...
                if (str(temp_Make_Name).replace('MAHINDRA SSANGYONG', 'MAHINDRA').replace('MARUTI SUZUKI',
                        ...
                        'MAHINDRA & MAHINDRA', 'MAHINDRA').replace('MARUTI SUZUKI', 'MARUTI').replace('  ',
                                                                                                      ' ').lower().strip()) and (
                        str(temp_Model_Name).upper().strip() == str(temp_MODEL_NAME).upper().strip()):
                    ...
                    str(temp_Fuel_Type2).replace('.', '').replace('-', '').lower().strip().__contains__(
                        ...
                        for i in temp_Var:
                            if i.upper().strip().__contains__(V_J):
                                if (str(temp_Fuel_Type2).replace('.', '').replace('-', '').lower().strip().__contains__(
                                    ...
                                    List_fi_VarI.append(temp_VARIANT_NAME)
                        if not temp_ftev:
                            Upperss = False
                            ...
                            for i in V_J:
                                ...
                                if i.upper().strip().__contains__(temp_Var.upper().strip()):
                                    ...
                                    List_fi21_VarI.append(temp_VARIANT_NAME)
                        if not Upperss:
                            ...
                            for i in range(len(temp_Var)):
                                ...
                                if len(temp_Var) >= 2:
                                    if V_J.upper().strip().__contains__(
                                        ...
                                        LessProb_VarI.append(temp_VARIANT_NAME)
                                    else:
                        ...
                        if not List_fi and not LessProb:
                            ...
                            List_fi2_VarI.append(temp_VARIANT_NAME)
                elif (str(temp_Make_Name).replace('MAHINDRA SSANGYONG', 'MAHINDRA').replace('.', '').replace('-',
                                                                                                             '').replace(
                        'AND', '&').replace('MARUTI SUZUKI', 'MARUTI').replace('MAHINDRA & MAHINDRA',
                                                                               'MAHINDRA').lower().strip() == str(
                        temp_MAKE_NAME).replace('.', '').replace('-', '').replace('AND', '&').replace(
                        'MAHINDRA & MAHINDRA', 'MAHINDRA').replace('MARUTI SUZUKI', 'MARUTI').replace('  ',
                                                                                                      ' ').lower().strip()) and (
                str(temp_MODEL_NAME).replace('.', '').replace('-', '').upper().strip().__contains__(
                        str(temp_Model_Name).replace('.', '').replace('-', '').upper().strip())):
                    ...
                    for i in temp_getVarf:
                        ...
                        if str(var_fin).upper().strip() == str(temp_MODEL_NAME).upper().strip():
                            if (str(temp_Fuel_Type2).replace('.', '').replace('-', '').lower().strip().__contains__(
                                ...
                                List_fi1_VarI.append(temp_VARIANT_NAME)
            ...
            if DirectMatc:
                ...
                List_fi_prob = []
                SET_IDs = List_fi
                Vari_list = List_fi_VarI
                Temp_var = temp_Variant_Name
                va,Out_P = get_fina_value(SET_IDs, Vari_list, Temp_var)
                ws_new_myra.cell(row=start_row_ws_i, column=Per).value = Out_P
                ws_new_myra.cell(row=start_row_ws_i, column=Set1_id).value = va
            elif List_fi21:
                print("List_fi21")
                List_fi2 = []
